...
# Interval screenshots are made in?
...
class App(object):
	...
	"""
	def __init__(self,n_generations,pop_size, graph=False, screenInterval=0):
		'''
		...
		if graph:
			...
			self.canvas_current = Canvas(self.window, height=350, width=350)
			self.canvas_best = Canvas(self.window, height=350, width=350)
			...
			#set up geometry
			# geo = self.window.geometry()	 #show current
	...
	def set_city_gcoords(self):
		...
		for city in list_of_cities:
			city.graph_x = (city.graph_x + (-1*min_x)+25)
			city.graph_y = (city.graph_y + (-1*min_y)+25)
		...
		for city in list_of_cities:
			...
			city.graph_y = 350 - (city.graph_y * stretch)
	...
	def GA_loop(self,n_generations,pop_size, graph=False):
		...
		for x in range(1,n_generations):
			...
			if graph:
				...
				#take screenshot in given interval (standard 0 => no screenshot)
				if self.screenInterval > 0:
					geo = [int(g) for g in re.split(regex, self.window.geometry())]
					if x % self.screenInterval == 0 or x == 1:
						img = ImageGrab.grab(bbox=(geo[2]+7, geo[3], geo[0]+geo[2]+7, geo[1]+geo[3]+27))
						img.save("screenshots/gen{0}.jpg".format(x))
		if graph:
			...
			if self.screenInterval > 0:
				geo = [int(g) for g in re.split(regex, self.window.geometry())]
				img = ImageGrab.grab(bbox=(geo[2]+7, geo[3], geo[0]+geo[2]+7, geo[1]+geo[3]+27))
				img.save("screenshots/gen{0}.jpg".format(n_generations))
...
def random_cities():
	...
	app = App(n_generations=k_n_generations,pop_size=k_population_size, graph=True, screenInterval=screenshotInterval)
